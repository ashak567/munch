...
import { getEnvelopeTextureCached } from '@/lib/assets-client'
...
export default function Envelope() {
  ...
  // Frequency checks: Only show surprise envelope if not dismissed today
  // and the user has completed at least one pick (stored in localStorage/session)
  useEffect(() => {
    if (typeof window === 'undefined') return
 
    const envelopeSeenToday = localStorage.getItem('munch-envelope-seen-today')
    if (!envelopeSeenToday) {
      // Simulate surprise delay: show envelope after 5 seconds on dashboard
  ...
  // Fetch custom envelope texture from Supabase Storage
  useEffect(() => {
    let active = true
    async function loadTexture() {
      try {
        const url = await getEnvelopeTextureCached()
        if (active && url) {
          setTextureUrl(url)
        }
      } catch (err) {
        console.warn('[Envelope] Error loading envelope texture:', err)
      }
    }
    loadTexture()
    return () => {
  ...
  const handleOpenSequence = () => {
    ...
    // Mark as seen for today to prevent repeated intrusion
    localStorage.setItem('munch-envelope-seen-today', new Date().toDateString())
  }
 
  const handleClose = () => {
    setIsOpen(false)
    setIsOpening(false)
    // Slide off and hide the envelope component entirely
        ...
        >
          ...
          >
            ...
            >
              ...
              {/* 4. HIDDEN LETTER CARD (Slides upward out of body) */}
              <motion.div
              ...
              >
                {/* Note Message Content (Handwritten aesthetic) */}
                <div className="space-y-2 flex-grow overflow-y-auto pr-1">
                  <motion.div
                    initial={{ opacity: 0 }}
                    animate={isOpen ? { opacity: 1 } : { opacity: 0 }}
                    transition={{ delay: 0.2, duration: 0.4 }}
                    className="font-serif italic text-xs leading-relaxed text-charcoal/90 pt-1"
                  >
                    ...
                    {"\"Hi friend, I noticed you had a lot on your mind today. Just a gentle reminder: you don't need a perfect choice to move forward. Take a slow breath and trust your rhythm. I'm right here with you.\""}
                  </motion.div>
                </div>
                ...
                >
                  ...
                  <span className="font-display font-black text-xs text-primary-dark">
                    Munch 🍀
                  </span>
                </motion.div>
              </motion.div>
 
              {/* 5. SMALL MASCOT INTEGRATION (Appears floating next to open letter) */}
              <AnimatePresence>
                {isOpen && (
                  <motion.div
                  ...
                  >
                    <Mascot character="munch" expression="happy" size="sm" className="drop-shadow-lg" />
                  </motion.div>
                )}
              </AnimatePresence>
